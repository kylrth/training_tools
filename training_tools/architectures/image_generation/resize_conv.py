"""Neural network with layers composed of a resize followed by a convolution, resulting
in a similar effect to a transpose convolution but without the overlapping effects.
...
class ResizeConvolution(tf.keras.layers.Conv2D):
    """This is designed to be a drop-in replacement for deconvolutions, without
    checkerboard artifacts. See https://distill.pub/2016/deconv-checkerboard/.

    Each resize step has the effect of two deconvolutions, so that the convolution
    brings the output to the desired shape as if a single deconvolution had been
    performed.
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        """Perform Conv2D initialization, and prepare arguments for
        tf.image.resize_images.

        Args in addition to arguments to the Conv2D layer:
            resize_method (tf.image.ResizeMethod): method to specify to
                                                   tf.image.resize_images.
        Args changed in call to the Conv2D layer:
            data_format: always set to 'channels_last' in order to use
                         tf.image.resize_images.
        """
...
def create_model(dataset, config, verbose):
    ...
    utils.v_print(
        verbose,
        "Creating resize-convolutional network with input "
    ...
    if config.layers < 0:
        # determine the number of resize-convolution layers that produces the output
    ...
    utils.v_print(
        verbose,
        "Using {} resize-conv layers each with kernel size {} and stride "
        "{} for an output of shape {}".format(
